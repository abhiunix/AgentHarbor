...
use analytics::claude::{
    claude_check_silent_credentials, claude_disconnect, claude_exchange_oauth_code,
    claude_import_from_keychain, claude_start_oauth, claude_store_manual_token,
};
use analytics::claude_v2::{
    export_claude_v2_csv, get_claude_v2_message_log, get_claude_v2_model_timeseries,
    get_claude_v2_overview, get_claude_v2_prompt_history, get_claude_v2_token_timeseries,
};
use analytics::commands::{
    copilot_poll_device_flow, copilot_start_device_flow, delete_provider_token,
    get_all_provider_analytics, get_all_provider_info, get_all_provider_status,
    get_provider_analytics, get_tray_summary, has_provider_token, save_provider_token,
    set_tray_active_provider, update_tray_tooltip,
};
use analytics::cursor_projects::{
    get_cursor_project_detail, get_cursor_projects_overview, start_cursor_in_project,
};
use analytics::cursor_v2::{
    disconnect_cursor_v2, get_cursor_v2_ai_commits, get_cursor_v2_cache_info,
    get_cursor_v2_composer_tabs, get_cursor_v2_connection_status, get_cursor_v2_model_usage,
    get_cursor_v2_overview, get_cursor_v2_request_breakdown, get_cursor_v2_team_spend,
    get_cursor_v2_usage_events, set_cursor_v2_cache_ttl,
};
...
use analytics::deepseek_plans::{list_deepseek_plans, list_deepseek_todo_groups};
use analytics::deepseek_prompts::{
    get_deepseek_prompt_history, get_deepseek_prompt_stats, search_deepseek_prompt_history,
};
use analytics::deepseek_transcripts::{
    list_deepseek_transcript_sessions, read_deepseek_transcript, search_deepseek_transcripts,
};
use analytics::deepseek_v2::{get_deepseek_v2_connection_status, get_deepseek_v2_overview};
use analytics::kimi_control::{
    get_kimi_config_tunables, get_kimi_control_settings, set_kimi_config_value,
    set_kimi_control_flag, set_kimi_default_model,
};
use analytics::kimi_instructions::{
    list_kimi_instruction_files, read_kimi_instruction, write_kimi_instruction,
};
use analytics::kimi_plans::{list_kimi_plans, list_kimi_todo_groups, read_kimi_plan};
use analytics::kimi_prompts::{
    build_kimi_resume_command, get_kimi_prompt_history, get_kimi_prompt_stats,
    search_kimi_prompt_history, start_kimi_session,
};
...
use analytics::kimi_v2::{
    get_kimi_v2_connection_status, get_kimi_v2_overview, get_kimi_v2_prompt_history,
};
use commands::agents::{delete_agent, import_agents_from_dir, preview_import_agents, save_agent};
use commands::ai_tracking::{
    get_ai_code_summary, get_ai_commit_scores, get_ai_file_type_breakdown,
    get_ai_tracking_model_breakdown, get_conversation_summaries,
};
use commands::backup::{
...
use commands::claude_metadata::{
    ...
    get_claude_settings_info, get_claude_todos_summary,
};
use commands::codex::{
...
use commands::codex_parity::{
    get_codex_control_snapshot, list_codex_models, read_codex_instructions, update_codex_control,
    update_codex_model_settings, write_codex_instructions,
};
use commands::config::{
    apply_claude_code_provider, get_author_id, get_claude_code_settings, get_settings,
    get_username, update_analytics_settings, update_deploy_settings, update_general_settings,
    update_registry_settings, update_settings, update_tray_settings,
};
use commands::cursor_hooks::{
    add_cursor_hook, list_cursor_hooks, remove_cursor_hook, save_cursor_hooks,
};
use commands::cursor_rules::{
    delete_cursor_rule, list_cursor_rules, list_global_cursor_rules, read_cursor_rule,
    write_cursor_rule,
};
use commands::custom::{
    delete_custom_capability, get_custom_capabilities_dir, list_custom_capabilities,
    save_custom_capability,
};
use commands::debate::{
    cancel_debate, check_debate_credentials, discover_project_plans, replace_plan_file,
    save_refined_plan, start_debate,
};
use commands::debate_history::{delete_debate, get_debate, list_debates};
use commands::deploy::{execute_deploy, preview_deploy};
use commands::discovery::{discover_plugins, discover_skills};
use commands::drift::{accept_drift, detect_drift, get_drift_diff, restore_drift};
use commands::extensions::{
    list_claude_plugins, list_cursor_extensions, list_cursor_plugins, toggle_claude_plugin,
};
use commands::gemini::{
    add_gemini_mcp_server, get_gemini_hooks, get_gemini_mcp_servers, list_gemini_agents,
    list_gemini_extensions, list_gemini_skills, read_gemini_agent, read_gemini_memory,
    read_gemini_project_memory, read_gemini_settings, remove_gemini_mcp_server,
    write_gemini_memory, write_gemini_project_memory, write_gemini_settings,
};
use commands::github_fetch::{
    fetch_github_skill, get_official_skills_index, get_openclaw_skills_index,
    search_openclaw_skills,
};
use commands::global_config::{
    add_global_mcp_server, discover_capabilities, get_claude_desktop_config_path,
    launch_claude_via_ollama, read_claude_desktop_config, read_claude_memory, read_claude_settings,
    read_global_config_raw, remove_global_mcp_server, test_ollama_connection,
    write_claude_desktop_config, write_claude_memory, write_claude_settings,
    write_global_config_raw,
};
use commands::importexport::{export_data, import_data, validate_import_data};
use commands::mcp_discovery::{discover_mcp_tools, discover_mcp_tools_http};
use commands::mcp_registry::{get_mcp_registry_popular, search_mcp_registry};
use commands::memory::{
    clear_agent_memory, clear_all_agent_memory, clear_all_global_memory, create_memory_file,
    delete_memory_file, list_agent_memory, list_global_agent_memory, list_memory_files,
    read_memory_file, read_project_memory, write_memory_file, write_project_memory,
};
use commands::model_routing::analyze_model_routing;
use commands::notes::{
    create_notes_file, create_notes_folder, delete_notes_entry, list_notes_entries,
    move_notes_entry, read_note_content, rename_notes_entry, write_note_content,
};
use commands::permissions::{
    get_claude_permissions, get_claude_project_permissions, get_cursor_permissions,
    update_claude_permissions, update_claude_project_permissions, update_cursor_permissions,
};
use commands::plans::{
    clear_hidden_debate_plans, delete_plan_file, get_todo_stats, hide_plan_from_debate,
    list_hidden_debate_plans, list_plans, list_project_plans, list_todos, read_plan,
    unhide_plan_from_debate,
};
use commands::presets::{
    add_capability_to_preset, delete_preset, get_presets, remove_capability_from_preset,
    save_preset, update_preset,
};
use commands::project::{
    add_recent_project, detect_adapters, get_global_config, get_recent_projects, open_in_app,
    remove_recent_project, select_project_folder,
};
use commands::projects::{
    add_project, get_all_projects, get_project_detail, get_project_installed_items,
    open_project_in_cursor, open_project_in_finder, open_project_in_terminal,
    open_project_in_vscode, record_deployment, remove_project, remove_project_item,
    start_claude_in_project, start_kimi_in_project,
};
use commands::prompt_history::{
    build_resume_command, get_prompt_history, get_prompt_stats, search_prompt_history,
...
use commands::secrets::{delete_secret, get_secret, get_secrets_count, list_secrets, store_secret};
use commands::session_stats::get_claude_session_stats;
use commands::sync::{
    get_community_registry_dir, get_sync_status, is_registry_polling_active,
    start_registry_polling, stop_registry_polling, sync_registry_now,
};
use commands::test_bridge::{test_bridge_read_cmd, test_bridge_write_result};
use commands::transcripts::{
    delete_transcript_backup, list_transcript_sessions, open_in_text_editor,
    prune_transcript_backups, read_transcript, read_transcript_raw, replace_in_transcript,
    save_transcript_raw, search_transcript_matches, search_transcripts, update_transcript_message,
};
use commands::usage::{list_projects_with_mcp, read_project_usage_files};
use commands::windsurf_rules::{
    delete_windsurf_rule, list_windsurf_rules, read_windsurf_legacy_rules,
    read_windsurf_rules_file, write_windsurf_legacy_rules, write_windsurf_rules_file,
};
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            get_codex_prompt_history,
            list_codex_transcript_sessions,
            read_codex_transcript,
            get_codex_plans_and_todos,
            get_codex_memory_status,
            read_codex_memory_document,
            // Debate
            start_debate,
            cancel_debate,
            replace_plan_file,
