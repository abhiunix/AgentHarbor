...
impl AntigravityAdapter {
    ...
    fn skills_dir(&self, project_path: &Path) -> PathBuf {
        project_path.join(".antigravity").join("skills")
    }
    ...
    fn deploy_skills(
        ...
        for skill in &skills {
            ...
            let frontmatter = build_antigravity_skill_frontmatter(skill);
            let body = skill
                .files
                .iter()
                .find(|f| {
...
fn build_antigravity_skill_frontmatter(skill: &Skill) -> String {
...
impl AgentAdapter for AntigravityAdapter {
    ...
    fn detect(&self, project_path: &Path) -> bool {
        project_path.join(".antigravity").exists()
    }
    ...
    fn diff(
        ...
        for skill in &skills {
            let artifact = skill.id.artifact_name(&skill.name);
            let skill_md_path = skills_dir.join(&artifact).join("SKILL.md");
            ...
            let change_type = if current.is_some() { ChangeType::Modify } else { ChangeType::Add };
