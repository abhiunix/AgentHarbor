...
const ADAPTER_CONFIGS: { id: string; name: string; color: string; globalPath: string }[] = [
  {
    id: "claude-code",
    name: "Claude Code",
    color: "#9333ea",
...
const CLAUDE_DESKTOP_CARD = {
  id: "claude-desktop",
  name: "Claude Desktop",
  color: "#d97706",
  globalPath: "Claude Desktop Config",
};

export function GlobalConfigView() {
  const [selectedAdapter, setSelectedAdapter] = useState<string>("claude-code");
  const [configs, setConfigs] = useState<AdapterGlobalConfig[]>([]);
  const [claudeDesktopMcpCount, setClaudeDesktopMcpCount] = useState(0);
  ...
  const loadGlobalConfigs = async () => {
    setLoading(true);
    try {
      const [loadedConfigs, desktopCount] = await Promise.all([
        Promise.all(
          ADAPTER_CONFIGS.map(async (adapter) => {
            try {
        ...
        readClaudeDesktopConfig()
          .then((raw) => {
            try {
              ...
              return typeof mcp === "object" && mcp !== null ? Object.keys(mcp).length : 0;
            } catch {
              return 0;
            }
          })
      ...
      setClaudeDesktopMcpCount(desktopCount);
    } finally {
      setLoading(false);
    }
  };

  const currentConfig = configs.find((c) => c.id === selectedAdapter);
  const allCards = [...ADAPTER_CONFIGS, CLAUDE_DESKTOP_CARD];

  return (
    <div className="h-full flex flex-col">
      <div className="p-6 border-b border-border">
        <h1 className="text-2xl font-semibold text-text-primary mb-2">Global Configuration</h1>
        <p className="text-text-muted">
          Manage global MCP servers and rules that apply across all projects.
        </p>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-text-muted mt-1">
              Global configuration applies to every project that uses this IDE adapter.
      ...
      <div className="p-6 flex-1 overflow-y-auto">
        <div className="flex gap-3 mb-6">
          {allCards.map((adapter) => {
            const isClaudeDesktop = adapter.id === "claude-desktop";
            const config = configs.find((c) => c.id === adapter.id);
            const mcpCount = isClaudeDesktop ? claudeDesktopMcpCount : (config?.mcpServers.length ?? 0);
            ...
            return (
              ...
              >
                ...
                <p className="font-medium text-text-primary text-left">{adapter.name}</p>
                <p className="text-xs text-text-muted text-left mt-1">
        ...
        {loading ? (
          ...
          <>
            {selectedAdapter === "claude-desktop" && (
              <ClaudeDesktopConfigSection className="mb-6" onRefresh={loadGlobalConfigs} />
            )}
            ...
            {currentConfig ? (
              <AdapterConfigSection
                ...
                onRefresh={loadGlobalConfigs}
              />
            ) : null}
          </>
        )}

        <GlobalAgentMemorySection />
      </div>
    </div>
  );
}
...
function ClaudeSettingsSection({ className = "" }: { className?: string }) {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      await writeClaudeSettings(content);
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    } finally {
      setSaving(false);
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded bg-accent-blue text-white text-xs font-medium hover:bg-accent-blue/90 disabled:opacity-50"
          >
            {saving ? "Saving..." : "Save"}
          </button>
        </div>
    ...
    <div className={`bg-app-card border border-border rounded-lg p-4 ${className}`}>
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-xs text-text-muted font-mono truncate max-w-md" title={path}>
            {path || "—"}
          </p>
        </div>
        <div className="flex gap-2">
...
function AdapterConfigSection({
  ...
  const handleRemoveServerClick = (name: string) => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            {config.hasConfig ? (
              ...
              <span className="text-xs px-2 py-1 rounded bg-white/10 text-text-muted">
                Not configured
              </span>
            )}
          </div>
          ...
          {config.mcpServers.length > 0 ? (
            <div className="space-y-2">
              {config.mcpServers.map((server) => (
                <div
                  key={server}
                  className="flex items-center gap-3 p-3 bg-app-card border border-border rounded-lg"
                >
            ...
            <div className="p-8 bg-app-card border border-border rounded-lg text-center">
              ...
              <p className="text-xs text-text-muted mt-2">
                Add a server below or edit Raw JSON.
              </p>
            </div>
          )}
        </div>
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded bg-accent-blue text-white text-xs font-medium disabled:opacity-50"
              >
      ...
      {addModalOpen && (
        <>
          ...
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md bg-app-card border border-border rounded-xl p-6 shadow-xl">
            ...
            {addModalOpen && rawError && (
              <p className="text-xs text-accent-red mb-3">{rawError}</p>
            )}
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button
                ...
                className="px-3 py-2 rounded border border-border text-text-primary text-sm hover:bg-app-card-hover"
              >
                Cancel
              </button>
              <button
                ...
                className="px-3 py-2 rounded bg-accent-blue text-white text-sm font-medium hover:bg-accent-blue/90 disabled:opacity-50"
              >
                Add
              </button>
            </div>
      ...
      <ConfirmDialog
        ...
        message={
          removeConfirm
            ? `Are you sure you want to remove the MCP server "${removeConfirm}"? This action cannot be undone.`
            : ""
        }
...
function GlobalAgentMemorySection() {
  ...
  const handleClearAll = async () => {
    if (!confirm("Clear ALL global agent memory? This cannot be undone.")) {
      return;
    }
    setClearing("all");
    try {
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-text-muted mt-0.5">
            User-scoped memory stored in ~/.claude/agent-memory/
          </p>
        </div>
        {memories.length > 0 && (
          <button
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {memories.map((memory) => (
            <div
              key={memory.path}
              className="flex items-center justify-between p-3 rounded-lg bg-app-bg"
            >
