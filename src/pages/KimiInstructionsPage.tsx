...
import {
  listKimiInstructionFiles,
  readKimiInstruction,
  writeKimiInstruction,
} from "../lib/tauri";
import type { KimiInstructionFile } from "../lib/tauri";
import { DebugPath } from "../components/common/DebugPath";

export function KimiInstructionsPage() {
  const [files, setFiles] = useState<KimiInstructionFile[]>([]);
  const [selected, setSelected] = useState<KimiInstructionFile | null>(null);
  ...
  const loadFiles = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await listKimiInstructionFiles();
      setFiles(result);
    } catch (e) {
  ...
  const openFile = useCallback(async (file: KimiInstructionFile) => {
    setSelected(file);
    setLoadingContent(true);
    setError(null);
    try {
      const text = await readKimiInstruction(file.abs_path);
      setContent(text);
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      await writeKimiInstruction(selected.abs_path, content);
      ...
      <div className="p-6 border-b border-border flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-text-secondary">
            Kimi has no global instructions file — AGENTS.md is per-project.
          </p>
      ...
      <div className="flex-1 overflow-hidden flex">
        <div className="w-72 flex-shrink-0 border-r border-border overflow-y-auto p-4 space-y-2">
          {loading ? (
            ...
            <p className="text-sm text-text-muted">No Kimi projects found.</p>
          ) : (
            files.map((file) => {
        ...
        <div className="flex-1 overflow-hidden p-6 flex flex-col gap-4">
          ...
          {!selected ? (
            <div className="h-64 flex items-center justify-center text-text-muted text-sm">
              Select a project to view or create its AGENTS.md.
            </div>
          ) : loadingContent ? (
            ...
            <>
              {!selected.exists && (
                <p className="text-xs text-text-muted">
                  This project has no AGENTS.md yet. Start typing and Save to create it.
                </p>
              )}
              <textarea
                value={content}
                ...
                placeholder="# AGENTS.md&#10;&#10;Add project-specific instructions for Kimi."
              />
            </>
          )}
        </div>
