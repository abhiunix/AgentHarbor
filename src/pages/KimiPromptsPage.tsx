...
import {
  getKimiPromptHistory,
  searchKimiPromptHistory,
  getKimiPromptStats,
  buildKimiResumeCommand,
  startKimiSession,
} from "../lib/tauri";
import type { KimiPromptHistoryEntry, KimiPromptHistoryStats } from "../lib/tauri";
...
export function KimiPromptsPage() {
  const [prompts, setPrompts] = useState<KimiPromptHistoryEntry[]>([]);
  const [stats, setStats] = useState<KimiPromptHistoryStats | null>(null);
  ...
  const fetchPrompts = useCallback(
    async (searchQuery: string, reset: boolean) => {
      try {
        ...
        let results: KimiPromptHistoryEntry[];
        if (searchQuery.trim()) {
          results = await searchKimiPromptHistory(searchQuery.trim());
        } else {
          const currentOffset = reset ? 0 : offset;
          const page = await getKimiPromptHistory(PAGE_SIZE, currentOffset);
  ...
  const handleLoadMore = async () => {
    ...
    setLoadingMore(true);
    try {
      const page = await getKimiPromptHistory(PAGE_SIZE, newOffset);
  ...
  const copyResumeCommand = async (entry: KimiPromptHistoryEntry, idx: number) => {
    if (!entry.session_id) return;
    try {
      const cmd = await buildKimiResumeCommand(entry.session_id, entry.project);
  ...
  const startSession = async (entry: KimiPromptHistoryEntry) => {
    if (!entry.session_id) return;
    try {
      await startKimiSession(entry.session_id, entry.project);
    } catch (e) {
  ...
  return (
    <div className="p-6 h-full flex flex-col">
      <div className="mb-6">
        ...
        <DebugPath path="~/.kimi/sessions" className="text-sm" />
        <p className="text-text-secondary text-sm">
          Browse and search your Kimi prompts
          {stats && (
                      ...
                      {entry.session_id && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            ...
                            title="Copy the kimi --resume command"
                          >
                            {copiedCmdIdx === idx ? (
                              <>
                                <svg
