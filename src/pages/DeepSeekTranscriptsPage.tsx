...
import {
  listDeepSeekTranscriptSessions,
  readDeepSeekTranscript,
  searchDeepSeekTranscripts,
} from "../lib/tauri";
import type { DeepSeekTranscriptSession, DeepSeekTranscriptMessage } from "../lib/tauri";
...
interface WorkspaceGroup {
  ...
  displayPath: string;
  sessions: DeepSeekTranscriptSession[];
  latestActivity: string | null;
}

function groupByWorkspace(sessions: DeepSeekTranscriptSession[]): WorkspaceGroup[] {
  const map = new Map<string, DeepSeekTranscriptSession[]>();
  ...
  for (const [workspacePath, items] of map) {
    ...
    groups.push({
      workspacePath,
      displayPath: shortenPath(workspacePath),
      sessions: sorted,
...
export function DeepSeekTranscriptsPage() {
  const [allSessions, setAllSessions] = useState<DeepSeekTranscriptSession[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [search, setSearch] = useState("");
  const [searching, setSearching] = useState(false);
  const [searchResults, setSearchResults] = useState<DeepSeekTranscriptSession[] | null>(
    null
  );
  const searchTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  const [expandedWorkspaces, setExpandedWorkspaces] = useState<Set<string>>(new Set());
  const [selectedSession, setSelectedSession] = useState<DeepSeekTranscriptSession | null>(
    null
  );
  const [messages, setMessages] = useState<DeepSeekTranscriptMessage[]>([]);
  ...
  useEffect(() => {
    ...
    listDeepSeekTranscriptSessions()
      .then((data) => {
        setAllSessions(data);
        if (data.length > 0) {
          const firstWorkspace = data[0]?.workspace_path;
          if (firstWorkspace) {
            setExpandedWorkspaces(new Set([firstWorkspace]));
          }
        }
      })
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(() => {
      searchDeepSeekTranscripts(query)
        .then((results) => {
          setSearchResults(results);
          const workspaceKeys = new Set(results.map((r) => r.workspace_path));
          setExpandedWorkspaces(workspaceKeys);
        })
  ...
  const handleSelectSession = (session: DeepSeekTranscriptSession) => {
    setSelectedSession(session);
    setMessages([]);
    setMessagesError(null);
    setSystemExpanded(false);
    setMessagesLoading(true);
    readDeepSeekTranscript(session.session_id, session.workspace_path)
      .then(setMessages)
  ...
  const groups = groupByWorkspace(baseSessions);
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-1/3 border-r border-border flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {groups.map((group) => {
            const isExpanded = expandedWorkspaces.has(group.workspacePath);
            return (
